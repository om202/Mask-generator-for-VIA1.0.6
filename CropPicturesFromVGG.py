...
def cropPictures(images, pathOriginal, pathSave):
    ...
    for picture in range(len(images[1])):
        print("For image {} ...".format(images[0][0][picture]))
        imgpath = pathOriginal+"/"+images[0][0][picture]
        original=Image.open(imgpath)
        mask=getMask(images[1][picture], images[2][picture], original)
